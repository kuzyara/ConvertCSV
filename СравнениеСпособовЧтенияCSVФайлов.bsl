...
Процедура ЧтениеCSVADOНаСервере()
	...
	// Вставить содержимое обработчика.
	т1 = ТекущаяУниверсальнаяДатаВМиллисекундах();
	ТаблицаРезультат = ПрочитатьCSVвТЗ_ADO(ИмяФайла);
	т2 = ТекущаяУниверсальнаяДатаВМиллисекундах();
	Сообщить("начало: " + т1);
	Сообщить("конец: " + т2);
	Сообщить("строк: " + КолСтрок);
...
Процедура ЧтениеCSVGameWithFireНаСервере()
	...
	// Вставить содержимое обработчика.
	т1 = ТекущаяУниверсальнаяДатаВМиллисекундах();
	ТаблицаРезультат = ПрочитатьCSV_GWF(ИмяФайла);
	т2 = ТекущаяУниверсальнаяДатаВМиллисекундах();
	Сообщить("начало: " + т1);
	Сообщить("конец: " + т2);
	Сообщить("строк: " + КолСтрок);
	...
	Сообщить("GWF, строк в секунду:" + КолСтрок/(т2-т1)*1000);КонецПроцедуры
...
// тестировалось на файлах в этой папке
