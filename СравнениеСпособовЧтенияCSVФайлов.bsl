...
Процедура Команда3НаСервере(Текст)
	...
	ТаблицаРезультат = ПрочитатьCSVФайлВТаблицу(ИмяФайла, Разделитель);
	...
	ТабДок = ДанныеТаблицыЗначенийВТабличныйДокумент(ТаблицаРезультат);
КонецПроцедуры
...
Процедура Команда4НаСервере()
	...
	т1 = ТекущаяУниверсальнаяДатаВМиллисекундах();
	ТаблицаРезультат = ПрочитатьCSVвТЗ(ИмяФайла, Разделитель);
	т2 = ТекущаяУниверсальнаяДатаВМиллисекундах();
...
&НаСервере
Функция ПолучитьТаблицуДанныхЧерезREADeR(ИмяФайла_CSV,Разделитель = ";",ПерваяСтрокаСодержитЗаголовки = Ложь) 
	...
	МассивПолейCSV = CSV_Reader(ЧтениеТекста, Разделитель);
	...
	Если ПерваяСтрокаСодержитЗаголовки Тогда
		...
		КонецЦикла;
		МассивПолейCSV = CSV_Reader(ЧтениеТекста, Разделитель);// Читаем вторую строку
...
Процедура Команда5НаСервере()
	...
	// Вставить содержимое обработчика.
	т1 = ТекущаяУниверсальнаяДатаВМиллисекундах();
	ТаблицаРезультат = ПолучитьТаблицуДанныхЧерезREADeR(ИмяФайла, Разделитель);
	т2 = ТекущаяУниверсальнаяДатаВМиллисекундах();
	Сообщить("строк: " + КолСтрок);
...
#Область ЧтениеADODB_ПоПравилам_1_2_3_4_5_6_7
...
&НаСервере
Процедура ЧтениеCSVADOНаСервере()
	ТекстДок = Новый ТекстовыйДокумент;
	ТекстДок.Прочитать(ИмяФайла);
	Текст = ТекстДок.ПолучитьТекст();
	КолСтрок = СтрЧислоСтрок(Текст);
	...
	// Вставить содержимое обработчика.
	т1 = ТекущаяУниверсальнаяДатаВМиллисекундах();
	ТаблицаРезультат = ПрочитатьCSVвТЗ_ADO(ИмяФайла, Разделитель);
	т2 = ТекущаяУниверсальнаяДатаВМиллисекундах();
	Сообщить("строк: " + КолСтрок);
	Сообщить("ADO, мс:" + (т2-т1));
	Сообщить("ADO, сек:" + (т2-т1)/1000);
	Сообщить("ADO, сек на 1тыс:" + (т2-т1)/1000 / (КолСтрок/1000));
	Сообщить("ADO, строк в секунду:" + КолСтрок/(т2-т1)*1000);
	
	ТабДок = ДанныеТаблицыЗначенийВТабличныйДокумент(ТаблицаРезультат);
КонецПроцедуры

&НаКлиенте
Процедура ЧтениеCSVADO(Команда)
	ЧтениеCSVADOНаСервере();
КонецПроцедуры

#КонецОбласти

#Область ЧтениеGameWithFire_ПоПравилам_1_2_3_4_5_6_7

&НаСервере
Функция ПрочитатьCSV_GWF(ИмяФайла, Разделитель=";")
	ФайлДанных = Новый Файл(ИмяФайла);
    ...
    strQuery = "SELECT * FROM [" + ФайлДанных.Имя + "]";
    strConn = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + ФайлДанных.Путь + ";Extended Properties=""text;HDR=No""";
    adOpenStatic = 3;
    adLockOptimistic = 3;
    adCmdText = 1;
...
&НаСервере
Процедура ЧтениеCSVGameWithFireНаСервере()
	ТекстДок = Новый ТекстовыйДокумент;
	ТекстДок.Прочитать(ИмяФайла);
	Текст = ТекстДок.ПолучитьТекст();
	КолСтрок = СтрЧислоСтрок(Текст);
	...
	// Вставить содержимое обработчика.
	т1 = ТекущаяУниверсальнаяДатаВМиллисекундах();
	ТаблицаРезультат = ПрочитатьCSV_GWF(ИмяФайла, Разделитель);
	т2 = ТекущаяУниверсальнаяДатаВМиллисекундах();
	Сообщить("строк: " + КолСтрок);
	Сообщить("GWF, мс:" + (т2-т1));
	Сообщить("GWF, сек:" + (т2-т1)/1000);
	Сообщить("GWF, сек на 1тыс:" + (т2-т1)/1000 / (КолСтрок/1000));
	Сообщить("GWF, строк в секунду:" + КолСтрок/(т2-т1)*1000);
	
	ТабДок = ДанныеТаблицыЗначенийВТабличныйДокумент(ТаблицаРезультат);
КонецПроцедуры

&НаКлиенте
Процедура ЧтениеCSVGameWithFire(Команда)
	#Если ТолстыйКлиентОбычноеПриложение ИЛИ ТолстыйКлиентУправляемоеПриложение Тогда
		ЧтениеCSVGameWithFireНаСервере();
	#Иначе
...
Функция ДанныеТаблицыЗначенийВТабличныйДокумент(ДанныеВТабличныйДокумент)
	...
	Для Каждого ТекКолонка Из ДанныеВТабличныйДокумент.Колонки Цикл
		Таблица.Колонки.Добавить(ТекКолонка.Имя, ОписаниеТиповС);
	КонецЦикла;
	Для Каждого ТекСтрока Из ДанныеВТабличныйДокумент Цикл
